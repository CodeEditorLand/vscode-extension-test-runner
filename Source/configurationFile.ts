		...
		return await new Promise<T>((resolve, reject) => {
			...
			p.on("close", (code) => {
				...
				try {
					...
					reject(
						new ConfigProcessReadError(`invalid JSON: ${joined}`),
					);
				}
			});
		});
...
export const isDesktopConfig = (
	cfg: TestConfiguration,
): cfg is IDesktopTestConfiguration =>
	...
	public includesTestFile(uri: vscode.Uri) {
		...
		for (const [i, patterns] of this.patterns.entries()) {
			...
			for (let { glob, value } of patterns) {
				...
				if (
					glob
						? minimatch(file, value)
						: value === path.normalize(file)
				) {
					matched = !negated;
				}
			}
