 ...
 */
export const extractWithEvaluation = (
	file: string,
	code: string,
	...
	const replacedGlobals = new Map<string, () => unknown>([
		...
		[
			"require",
			() => (mod: string) =>
				allowedNodeModules.some(
					(m) => m === mod || mod.startsWith(m + "/"),
				)
				...
				if (typeof value === "function") {
					return function (...args: any[]) {
						return value.apply(
							target,
							args.map((arg) =>
								placeholders.has(arg) ? undefined : arg,
							),
						);
					};
				}
	...
	const contextObj = new Proxy({} as any, {
		get(target, prop, _receiver) {
			if (symbols.suite.includes(prop as string)) {
				...
				prop in globalThis ||
				replacedGlobals.has(prop as string)
			) {
				return (
					replacedGlobals.get(prop as string)?.() ??
