...
export class TestRunner {
	...
	public makeHandler(
		...
		return async (request) => {
			...
			const { args, compiledFileTests, leafTests } =
				...
				onLine: (line) => {
					...
					switch (parsed[0]) {
						...
						case MochaEvent.Fail: {
							...
							const locationProm = tryDeriveStackLocation(
								this.smStore,
								rawErr,
								tcase!,
							);
			...
			run.appendOutput(
				`${styles.inverse.open} > ${styles.inverse.close} ${
					wrapper
						? `${wrapper instanceof Array ? wrapper.join(" ") : wrapper} `
						: ""
				}vscode-test ${spawnOpts.args.join(" ")}\r\n`,
			);

			const promise = (
				this.queues.get(userDataDir) || Promise.resolve()
			).then(async () => {
				try {
					if (debug) {
			...
			ds.add(
				vscode.debug.registerDebugAdapterTrackerFactory("*", {
					createDebugAdapterTracker(session) {
						if (
							session.configuration.__extensionSessionKey !==
								sessionKey &&
	...
	private async prepareArguments(
		...
		for (const test of include) {
			...
			if (data.type === ItemType.Test || data.type === ItemType.Suite) {
				grepRe.push(
					escapeRe(getFullName(test)) +
						(data.type === ItemType.Test ? "$" : " "),
				);
			}
...
class CompiledFileTests {
	 ...
	 */
	private getPathInTestItems(
		items: Set<vscode.TestItem>,
...
async function deriveSourceLocation(
	...
	const value =
		...
		new vscode.Location(
			fileUri,
			new vscode.Position(Number(line), Number(col)),
		);
